...
async fn list_folders(
    ...
    RichMessage::info(body.trim_end()).with_title(t(lang, "Working Folder", "工作目录"))
}
...
async fn select_folder_by_index(
    ...
    RichMessage::info(format!("{} ({})", folder.name, folder.path)).with_title(t(
        lang,
        "Folder Selected",
        "已选择目录",
    ))
...
async fn select_folder_by_path(
    ...
    RichMessage::info(format!("{} ({})", entry.name, entry.path)).with_title(t(
        lang,
        "Folder Selected",
        "已选择目录",
    ))
...
async fn list_agents(
    ...
    RichMessage::info(body.trim_end()).with_title(t(lang, "Agent Selection", "选择 Agent"))
}
...
async fn select_agent_by_index(
    ...
    RichMessage::info(at.to_string()).with_title(t(lang, "Agent Selected", "已选择 Agent"))
}
...
pub async fn handle_task(
    ...
    RichMessage::info(format!("[{}] #{} @ {}", agent_type, conv.id, folder.name,)).with_title(t(
        lang,
        "Task Started",
        "任务已启动",
    ))
...
pub async fn handle_resume(
    ...
    let conv = match conversation_service::get_by_id(db, conversation_id).await {
        Ok(c) => c,
        Err(_) => {
            return RichMessage::info(t(lang, "Conversation not found.", "会话不存在。"));
        }
    };
...
pub async fn handle_permission_response(
    ...
    let connection_id = match &ctx.current_connection_id {
        Some(id) => id.clone(),
        None => {
            return RichMessage::info(t(lang, "No active session.", "没有活跃的会话。"));
        }
    };
        ...
        .await
    {
        return RichMessage::error(format!(
            "{}{e}",
            t(lang, "Failed to respond to permission: ", "权限响应失败: ")
        ));
    }
...
async fn list_recent_sessions(db: &DatabaseConnection, lang: Lang, prefix: &str) -> RichMessage {
    ...
    if recent.is_empty() {
        return RichMessage::info(t(lang, "No conversations found.", "暂无会话记录。"))
    ...
    RichMessage::info(body.trim_end()).with_title(t(lang, "Recent Conversations", "最近会话"))
}
