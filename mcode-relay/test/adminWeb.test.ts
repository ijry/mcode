...
describe("relay admin web console", () => {
  it("serves the admin shell and allowlisted assets", async () => {
    ...
    expect(shell.text).toContain("tenant-filter-form")
    expect(shell.text).toContain("export-audit-jsonl")
    ...
    expect(script.text).toContain("mcodeAdminTenantId")
    expect(script.text).toContain("/v1/admin/audit-events/export")
    expect(script.text).toContain("URLSearchParams")
