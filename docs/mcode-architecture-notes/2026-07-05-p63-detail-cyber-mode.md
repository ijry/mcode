...
`mcode-app` 的会话详情页把原先的实验性炫酷模式升级为主题体系，当前支持 `default`、`matrix`、`sweet` 三个详情页主题。主题状态使用 `mcode_detail_theme_v1` 持久化，不修改 ACP、SQLite、路由或 runtime store 数据结构；旧的 `mcode_detail_cyber_mode_v1=true` 会自动迁移到 `matrix`。
...
- 在 `matrix` 和 `sweet` 主题下隐藏自定义背景图，但不清除原有背景配置

纯逻辑放在 `detailCyberMode.ts`。`matrix` 主题的整页绿色数字雨由 `ConversationDetailCyberRain.vue` 渲染，`sweet` 主题的漂浮泡泡背景由 `ConversationDetailSweetBubbles.vue` 渲染。实时消息解码效果只属于 `matrix` 主题，并且只对当前活动 pane 中、状态为 `streaming` 的 assistant 最新文本段开启。

主题体系不再把实验模式视为默认主题的颜色 tint，而是独立的详情页皮肤。页面 shell 在 `matrix` 与 `sweet` 下都会覆盖 page、navbar、tabs 等 inline/prop 样式，避免 `upThemeCardStyle` 或 `up-navbar bgColor` 把界面重新刷回默认浅色。
...
`index.vue` 根据这些输入推导页面阶段，并把 `detailTheme`、`cyberEffectPhase` 逐层传给 `ConversationDetailInteractivePane`、`ConversationDetailReadonlyTimeline` 与 `MessageBubble`。`MessageBubble` 本地使用定时器驱动 0/1 解码覆盖层，不向上写回任何业务状态；`sweet` 主题不启用该解码层。
...
- `matrix` 顶部导航和手机状态栏统一为纯黑背景，状态栏图标切换为浅色；`sweet` 统一为浅粉玻璃背景和深色状态栏图标；默认主题保持当前 uview 主题色。
