...
- `mcode-app` 连接运行时、配置码导入、详情页连接解析、driver 选择和连接 key 匹配只接受 `version: 2` 连接记录。
- `mode`、`url`、`relaySession`、`targetType` 等 v1/过渡字段不得再作为运行时兼容别名读取。
- 唯一例外是连接首页启动时调用 `migrateLegacyStoredConnectionsToV2()`，把本机 `mcode_connections` 中的旧 `mode = direct|relay` 记录一次性转换为 `targetAgent = codeg` 的 v2 记录；转换后立即写回 v2 数组，其他页面和服务不得复制这段迁移逻辑。
- 新二维码 / 配置码只写 `version: 2`；导入 v1 配置码必须失败并提示不支持。
...
- relay 侧的 `PairingStore` 已保存 `targetAgent`、`capabilities`、`protocolVersion`；P42 后 `pair_offer` 和 `desktop_hello` 必须显式携带 `targetAgent` 与 `protocolVersion`，缺失字段的旧帧会被忽略，不再按 `codeg` / protocol `1` 补默认值。
...
- web/uni-app 新增连接表单已经使用 `routeMode`、`targetAgent`、`gatewayProvider` 作为主状态；P42 后不再为连接表单或配置码保留 `mode`、`url`、`relaySession` 兼容别名。
...
- P42 后 app 运行时是 v2-only。旧连接只在连接首页进入时做一次本地迁移，迁移结果必须是 `ConnectionRecordV2`。
- 非首页模块如果读到 v1 连接形态必须视为无效数据，不能再自动推断为 `codeg`、`direct` 或 `gateway`。
- pair/refresh 返回缺少 `targetAgent` 的旧网关响应必须失败；缺少 `targetAgent` / `protocolVersion` 的 desktop 上游帧和 relay 持久化 target 快照必须被丢弃。
- 原生 iOS/Android 不要复制旧字段别名兼容；只可在本地启动或连接首页实现同等的一次性旧连接迁移。
...
- 二维码 / 配置码只支持 `version: 2` 写入和导入；`version: 1` 配置码必须拒绝。只有本机旧 `mcode_connections` 可在连接首页做一次性迁移。
...
when restored target snapshots already contain P42-required v2 fields
(`targetAgent` and `protocolVersion`); tenant-less v2 records fall back to the
`default` tenant.
...
- Old JSON snapshots without a `tenants` array still restore with
  `tenantId = "default"` only when their targets already include
  `targetAgent` and `protocolVersion`.
...
- The conversation detail page preserves v2 connection context when opened from
  ...
  and `targetProfile`. P42 后 detail lookup only accepts v2 keys such as
  `codeg::gateway::https://...`; legacy keys such as `relay::https://...` are no
  longer recognized outside the homepage migration boundary.
- Sending a draft resolves the current `targetAgent` before building prompt
  blocks. Missing v2 connection metadata defaults to Codeg only as a safe
  runtime fallback; explicit non-Codeg targets can send text but reject
  attachments until their own adapter is implemented.
