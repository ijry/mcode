...
The mcode account integration now has a built-in production origin:
`https://getmcode.lingyun.net`. `mcode-app/src/services/xycloudAuth.ts`
...
For each request, the service resolves the origin in this order:

1. Use a non-empty normalized runtime override from `setXycloudBaseUrl()`.
2. Fall back to `https://getmcode.lingyun.net`.

Resolved origins are trimmed and stripped of trailing slashes before the API
path is appended.
...
configured value now use the production account API instead of failing with
"请先配置 Xycloud 后端地址".
...
Native clients should centralize account API origin resolution in their account
auth client, not in individual screens. Use `https://getmcode.lingyun.net` as
the default origin and allow an app-config override for development or private
deployments. Normalize the chosen origin by trimming whitespace and removing
trailing slashes before appending endpoint paths.
